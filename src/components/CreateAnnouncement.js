...
function CreateAnnouncement() {
  ...
  useEffect(() => {
    const fetchProfessorClasses = async () => {
      ...
      if (!token || !loggedInUser || loggedInUser.role !== "professeur") {
        setMessage(
          "Vous devez être connecté en tant que professeur pour gérer les annonces."
        );
        setProfessorClasses([]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!user || user.role !== "professeur") {
      setMessage(
        "Accès non autorisé. Seuls les professeurs peuvent créer des annonces."
      );
      setIsLoading(false);
      return;
    }
        ...
        {isFetchingClasses ? (
          ...
          !isFetchingClasses && (
            <div className="warning-message">
              ...
              <p>
                {message ||
                  "Vous n'avez pas encore créé de classes. Veuillez créer une classe avant de pouvoir publier des annonces."}
              </p>
            </div>
          )
        )}
